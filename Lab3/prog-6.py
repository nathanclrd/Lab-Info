...
def mua_2d(depart, temps_depart, vitesse_initiale, acceleration_verticale,
           temps_maintenant):
    xt = depart[0] + vitesse_initiale[0]*(temps_maintenant-temps_depart)
    yt = depart[1] + vitesse_initiale[1]*(temps_maintenant-temps_depart) + (acceleration_verticale*(temps_maintenant-temps_depart)**2)/2
    return (xt,yt)
...
def calculer_vitesse(depart, angle, cible, acceleration_verticale):
    ...
    v0 = 0
    if angle <= math.atan2(delta_y,delta_x):
        tir_est_possible = True
        v0 = delta_x/(math.cos(angle)*math.sqrt((2/acceleration_verticale)*(delta_y-math.tan(angle)*delta_x)))
        print(depart[0])
    return (tir_est_possible, v0)

# *** A MODIFIER *********************************************************
def calculer_impact(depart, angle, hauteur_sol, vitesse,acceleration_verticale):
    xs = depart[0]+(vitesse*math.cos(angle)*((-vitesse*math.sin(angle)+math.sqrt((vitesse*math.sin(angle))**2-2*acceleration_verticale*(depart[1]-hauteur_sol)))/acceleration_verticale))
    return xs
...
def tri_projectiles(projectiles):
    temps_maintenant = pygame.time.get_ticks()
    return list(filter(lambda x: x['temps_depart']> temps_maintenant - 4000, projectiles))
