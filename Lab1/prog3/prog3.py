...
def initialiser_calculs():
    global altitude,temps,vitesse
    altitude = 0
    temps = 0
    vitesse =0
    return altitude,temps,vitesse
...
def calculer_vitesse_acceleration(altitude_avion, temps_maintenant):
    ...
    vy = -(altitude_avion-altitude)/(temps_maintenant-temps)
    ay = (vy-vitesse)/(temps_maintenant-temps)
    altitude = altitude_avion
...
def mrua_1d(depart, temps_depart, acceleration, temps_maintenant):
    ...
    return depart+acceleration/2*(temps_maintenant-temps_depart)**2

# *** A MODIFIER *********************************************************

def calculer_tir(altitude_larguage, altitude_cible, acceleration,prochain_temps_cible, temps_maintenant):

    temps = math.sqrt((2*(altitude_cible-altitude_larguage))/acceleration)
    tir_est_possible = temps_maintenant + temps < prochain_temps_cible
    temps_tir = prochain_temps_cible-temps
...
while True:
    ...
    vitesse_v, acceleration_v = calculer_vitesse_acceleration(altitude_avion,temps_maintenant)
